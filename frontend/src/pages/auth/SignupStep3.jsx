...
const SignupStep3 = ({ nextStep, prevStep }) => {
  ...
  const handleComplete = (e) => {
    ...
    nextStep({ interests: selectedIds }); // 순수 데이터만 전달
  };
          ...
          {interests.map((item) => (
            <div
              ...
              className={`relative p-6 rounded-2xl cursor-pointer transition-all border-2 flex flex-col items-center justify-center gap-3 ${
                selectedIds.includes(item.id) ? 'border-[#556B2F] bg-[#F0F4E8]' : 'border-gray-50 bg-gray-50 hover:bg-gray-100'
              }`}
            >
              {selectedIds.includes(item.id) && (
                <div className="absolute top-3 right-3 bg-[#556B2F] text-white rounded-full p-1"><Check size={12} /></div>
              )}
              <div className={`p-4 rounded-full ${item.color}`}><item.icon size={28} /></div>
